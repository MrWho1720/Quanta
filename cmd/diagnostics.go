...
func newDiagnosticsCommand() *cobra.Command {
	command := &cobra.Command{
		Use:   "diagnostics",
		Short: "Collect and report information about this Quanta instance to assist in debugging.",
...
// diagnosticsCmdRun collects diagnostics about quanta, its configuration and the node.
// We collect:
// - quanta and docker versions
...
func diagnosticsCmdRun(*cobra.Command, []string) {
	...
	fmt.Fprintln(output, "Quantum Quanta - Diagnostics Report")
	printHeader(output, "Versions")
	fmt.Fprintln(output, "               Quanta:", system.Version)
	if dockerErr == nil {
	...
	printHeader(output, "Quanta Configuration")
	...
	printHeader(output, "Latest Quanta Logs")
	if diagnosticsArgs.IncludeLogs {
		p := "/var/log/quantum/quanta.log"
		if cfg != nil {
			p = path.Join(cfg.System.LogDirectory, "quanta.log")
		}
