...
import (
	...
	"github.com/quantum/quanta/environment/docker"
...
func GetHandler(s *server.Server, w http.ResponseWriter, r *http.Request, c *gin.Context) (*Handler, error) {
	upgrader := websocket.Upgrader{
		...
		CheckOrigin: func(r *http.Request) bool {
			...
			panelLoc := strings.TrimRight(config.Get().PanelLocation, "/")
			if o == panelLoc {
				return true
			}
			for _, origin := range config.Get().AllowedOrigins {
				if origin == "*" || strings.TrimRight(origin, "/") == o {
					return true
				}
			}
			return false
	...
	case AuthenticationEvent:
		{
			...
			if !newConnection {
				...
				// https://github.com/quantum/panel/issues/2077
				return nil
			}
