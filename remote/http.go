...
type Client interface {
	...
	PushServerStateChange(ctx context.Context, uuid string, state string) error
}
...
func (c *client) requestOnce(ctx context.Context, method, path string, body io.Reader, opts ...func(r *http.Request)) (*Response, error) {
	...
	req.Header.Set("User-Agent", fmt.Sprintf("Quantum Quanta/v%s (id:%s)", system.Version, c.tokenId))
	req.Header.Set("Accept", "application/vnd.quantum.v1+json")
