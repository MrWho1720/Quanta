...
// exposed by the Quanta webserver.
...
// from Quanta to the Panel.
type RemoteQueryConfiguration struct {
	// The amount of time in seconds that Quanta should allow for a request to the Panel API
	...
	// Quanta instance. A single request is initially made to the Panel to get this number
...
type SystemConfiguration struct {
	...
	// can also be done.
	//
	// This timezone value is passed into all containers created by Quanta.
	...
	// Passwd controls the mounting of a generated passwd files into containers started by Quanta.
	Passwd struct {
		// Enable controls whether generated passwd files should be mounted into containers.
		//
		// By default this option is disabled and Quanta will not mount any
		...
		// This directory may be temporary as it will be re-created whenever Quanta is started.
		//
		// This path **WILL** be both written to by Quanta and mounted into containers created by
		// Quanta. If you are running Quanta itself in a container, this path will need to be mounted
	...
	MachineID struct {
		...
		// into containers.
		//
		// By default this option is enabled and Quanta will mount an additional
	...
	// impact system performance and cause massive I/O bottlenecks and high CPU usage for the Quanta
	// process.
	//
	...
	// If set to false Quanta will not attempt to write a log rotate configuration to the disk
...
type CrashDetection struct {
	...
	// Determines if Quanta should detect a server that stops with a normal exit code of
	// "0" as being crashed if the process stopped without any Quanta interaction. E.g.
...
type Backups struct {
	...
	// CompressionLevel determines how much backups created by quanta should be compressed.
	//
...
type Configuration struct {
	...
	// Determines if quanta should be running in debug mode. This value is ignored
	...
	// is only required by users running Quanta without SSL certificates and using internal IP
...
// EnsureQuantumUser ensures that the Quantum core user exists on the
...
// ConfigurePasswd generates required passwd files for use with containers started by Quanta.
...
func FromFile(path string) error {
	...
	c.Token = Token{
		ID:    os.Getenv("QUANTA_TOKEN_ID"),
		Token: os.Getenv("QUANTA_TOKEN"),
	}
	if c.Token.ID == "" {
...
// EnableLogRotation writes a logrotate file for quanta to the system logrotate
...
func EnableLogRotation() error {
	if !_config.System.EnableLogRotate {
		log.Info("skipping log rotate configuration, disabled in quanta config file")
		return nil
	}
    ...
    notifempty
    postrotate
        /usr/bin/systemctl kill -s HUP quanta.service >/dev/null 2>&1 || true
    endscript
}`)
	if err != nil {
		return err
...
func ConfigureTimezone() error {
	...
	if _config.System.Timezone == "" {
		...
		if err != nil {
			...
			// fall through to UTC to get Quanta booted at least.
