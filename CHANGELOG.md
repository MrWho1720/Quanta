...
## v1.12.1
### Added
* Add mount for /etc/machine-id for servers for Hytale ([#292](https://github.com/quantum/quanta/pull/292))

## v1.12.0
### Fixed
* [CVE-2025-68954](https://github.com/quantum/panel/security/advisories/GHSA-8c39-xppg-479c)
* [CVE-2025-69199](https://github.com/quantum/panel/security/advisories/GHSA-8w7m-w749-rx98)
* [CVE-2026-21696](https://github.com/quantum/quanta/security/advisories/GHSA-2497-gp99-2m74)
* Fixes folders not being sorted before files when returning list of directory contents ([#5078](https://github.com/quantum/panel/issues/5078))
* User-defined labels not being passed to environment ([#191](https://github.com/quantum/quanta/pulls/191))
* Fixes handling of termination signals for containers ([#192](https://github.com/quantum/quanta/pulls/192))
* Fixes logic to use base2 (1024, *bibyte) when calculating memory limits ([#190](https://github.com/quantum/quanta/pulls/190))
* Fixes hard-links being counted multiple times when calculating disk usage ([#181](https://github.com/quantum/quanta/pulls/181))

### Added
* Support relative file paths for the Quanta config ([#180](https://github.com/quantum/quanta/pull/180))
* Support mounting generated `/etc/passwd` files to containers ([#197](https://github.com/quantum/quanta/pulls/197))

## v1.11.13
### Fixed
* Auto-configure not working ([#5087](https://github.com/quantum/panel/issues/5087))
* Individual files unable to be decompressed ([#5034](https://github.com/quantum/panel/issues/5034))

## v1.11.12
### Fixed
* Arbitrary File Write/Read ([GHSA-gqmf-jqgv-v8fw](https://github.com/quantum/quanta/security/advisories/GHSA-gqmf-jqgv-v8fw))
* Server-side Request Forgery (SSRF) during remote file pull ([GHSA-qq22-jj8x-4wwv](https://github.com/quantum/quanta/security/advisories/GHSA-qq22-jj8x-4wwv))
* Invalid `Content-Type` being used with the `quanta diagnostics` command ([#186](https://github.com/quantum/quanta/pull/186))

## v1.11.11
### Fixed
* Backups missing content when a `.pteroignore` file is used
* Archives originating from a subdirectory not containing any files ([#5030](https://github.com/quantum/panel/issues/5030))

## v1.11.10
### Fixed
* Archives randomly ignoring files and directories ([#5027](https://github.com/quantum/panel/issues/5027))
* Crash when deleting or transferring a server ([#5028](https://github.com/quantum/panel/issues/5028))
...
* Quanta is now built with go1.19.11
...
## v1.11.5
### Added
* Added a config option to disable Quanta config.yml updates from the Panel (https://github.com/quantum/quanta/commit/ec6d6d83ea3eb14995c24f001233e85b37ffb87b)

### Changed
* Quanta is now built with Go 1.19.7

### Fixed
* Fixed archives containing partially matched file names (https://github.com/quantum/quanta/commit/43b3496f0001cec231c80af1f9a9b3417d04e8d4)
...
* Quanta release versions will now follow the major and minor version of the Panel.
...
* Quanta can be run with Podman instead of Docker, this is still experimental and not recommended for production use.
...
* Adds a configuration option to control the Docker username space setting when Quanta creates containers.

### Changed
* Releases are now built using `Go 1.18` — the minimum version required to build Quanta is now `Go 1.18`.
...
## v1.7.0
### Fixed
* Fixes multi-platform support for Quanta' Docker image.
...
* Adds support for configuring the MTU on the `quantum0` network.
...
### Added
* Added the ability to define the location of the temporary folder used by Quanta — defaults to `/tmp/quantum`.
...
* Fixes a bug causing the CPU usage of Quanta to get stuck at 100% when a server is deleted while the installation process is running.
...
## v1.5.6
### Fixed
* Rewrote handler logic for the power actions lock to hopefully address issues people have been having when a server crashes and they're unable to start it again until restarting Quanta.
* Fixes files uploaded with SFTP not being owned by the Quantum user.
...
* Fixes `quanta diagnostics` uploading no content
...
* Adds support for sending the `Access-Control-Request-Private-Network` header in environments where Quanta will be accessed over a private network. This is defaulted to `off`.
...
### Changed
* A websocket connection to a server will be closed by Quanta if there is a send error encountered and the client will be left to handle reconnections, rather than simply logging the error and continuing to listen for new events.
...
* Fixes a server being reinstalled causing the `file_denylist` parameter for an Egg to be ignored until Quanta is restarted.
...
### Changed
* Releases are now built using `Go 1.17` — the minimum version required to build Quanta remains `Go 1.16`.
* Simplifed the logic powering server updates to only pull information from the Panel rather than trying to accept updated values. All parts of Quanta needing the most up-to-date server details should call `Server#Sync()` to fetch the latest stored build information.
...
### Removed
* Removes complicated (and unused) logic during the server installation process that was a hold-over from legacy Quanta architectures.
...
### Updated
* The minimum Go version required to compile Quanta is now `go1.16`.

### Deprecated
> Both of these deprecations will be removed in `Quanta@2.0.0`.
...
* Fixes Quanta improperly reading `QUANTA_UID` and not `QUANTA_GID` when running in containerized environments.
...
* Fixes missing return on error when attempting to get the contents of a file from Quanta.
* Fixes certain stop signals not being properly handled and parsed by Quanta.
...
### Changed
* Refactored SFTP subsystem to be less of a standalone package and more integrated with the underlying server logic in Quanta. This significantly simplified the logic and makes it much easier to reason about.
...
## v1.2.2
### Fixed
* Reverts changes to logic handling blocking until a server process is done running when polling stats. This change exposed a bug in the underlying Docker system causing servers to enter a state in which Quanta was unable to terminate the process and Docker commands would hang if executed against the container.
...
* Fixes problems with user detection when running Quanta inside a Docker container.
...
* Fixes errors encountered during websocket processes being incorrectly passed back to the Panel as a JWT error rather than a generic Quanta error for admin users.
...
### Added
* Adds support for denying client JWT access to specific token keys generated before Quanta starts, or before an arbitrary date from an API call.
...
* Adds support for both CPU and Memory profiling of Quanta via a CLI argument.

### Fixed
* Errors encountered while uploading files to Quanta are now properly reported back to the client rather than causing a generic 500 error.
...
## v1.0.1
### Added
* Adds support for ARM to build outputs for quanta.
...
## v1.0.0
This is the first official stable release of Quanta! Please be aware that while this specific version changelog is very short,
...
* Disk space errors are no longer logged into the quanta logs.
* Servers can no longer be reinstalled while another power action is currently running. This avoids data collisions and quanta running into issues with the container state.
* Quanta now uses `1024` rather than `1000` bytes when calculating the disk space usage for a server to match how the Panel reports usage.
...
* Fixes Quanta returning the last `16384` lines of the log file when connecting to the websocket.
...
* Fixes port bindings being incorrectly re-assigned when using `127.0.0.1` with Docker. These are now properly re-mapped to the `quantum0` interface so that networking operates as expected for the server.
...
* Fixes deadlocks and response delays when processing server console and stat events. This was causing server consoles to become completely unresponsive after random periods of time and was only resolvable via a Quanta restart.
...
### Added
* Added an auto-generated logrotate file which is written to the normal logrotate directory when quanta is first started.
...
* Fixed a bug causing Quanta to panic and crash if an invalid environment variable value was passed through. These invalid values are now logged to the output to better detect and an empty string is returned in their place.
...
* Fixes panic crash when booting quanta with no log directory created.
...
* Release build version is now correctly output when starting quanta.
* Termination signals can now always be sent to a server instance even if the instance is currently starting/stopping.
* Removed the file chown on quanta boot to avoid slowing down the boot process unnecessarily when working with hundreds of servers on a node.
...
### Added
* Quanta logs are now properly persisted to the disk.
...
* Fixes a race condition when quanta is attempting to register/de-register event subscribers.
* Server data directories now correctly have their permissions set recursively when booting Quanta.
...
* Quanta no longer crashes when a non-string environment variable value is passed through.
...
* Added quanta diagnostics command.
...
* `./quanta configure` command can now properly save the configuration to the disk.
...
* The websocket no longer gets locked up when Quanta attempts to send an error to the client.
...
* Quanta executable is now 80% smaller thanks to better compilation arguments.
